...
def parse_args():
    ...
    parser.add_argument('--dataset_id', type=int, default=5)
    parser.add_argument('--gpu', type=str, default='0', help='assign multi-gpus by comma concat')  
    parser.add_argument('--num_classes', type=int, default=7)
    parser.add_argument('-c', '--checkpoint', type=str, default=r'D:\DL_Study\Expression_Project\Video_FER\专用可视化权重\DFEW\sota\best_val_acc_uar__epoch32_acc0.72622_uar0.64016_val_acc_uar1.3663800117111204.pth', help='Pytorch checkpoint file path')
...
def test():
    ...
    test_dataset = test_data_loader(args.dataset_id)
    ...
    with torch.no_grad():
        bingo_cnt = 0
        model.eval()
        for _, (imgs, targets, images_path) in tqdm(enumerate(tqdm(test_loader))):
